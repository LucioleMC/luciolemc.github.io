...
function patternsHandler(event) {
	if (event.target.tagName == 'I'){
		...
		if (event.target.children[0].tagName == 'I'){
			var ptn = event.target.children[0].getAttribute('ptn');
			var clr = _colors[_color];
			newPattern(ptn, clr);
			_preview.innerHTML = '';
			setUrl();
...
var _savedTemp = localStorage.getObject('bn');
	if (_savedTemp.length > 0) {
		...
		_urlGal.value = 'http://system42.fr/n/banners/?='+hash;
	}else{
		_urlGal.value = '';
	}
}
...
var hiddens = scrCon.querySelectorAll('.button, #saved-gal-cont, .layer-move, .layer-del, .layer-vis, .layer-drag, .header-button');
	...
	_scrlazy = document.getElementsByClassName('canvas-cont')[0].className;
	document.getElementsByClassName('canvas-cont')[0].classList.remove('epic');
	html2canvas(scrCon,{
		...
		var elem = document.createElement('a');
			...
			document.getElementsByClassName('canvas-cont')[0].className = _scrlazy;
		}
    })
}
...
if (supports_html5_storage()) {
	...
	function loadLocal() {
		...
		if (localStorage.compact == 'on') {
			document.getElementById('bcompact').classList.add('on');
			document.getElementById('bcompact').parentNode.classList.add('compact');
		}
		if (localStorage.epic == 'on') {
			document.getElementById('bepic').classList.add('on');
			document.getElementsByClassName('canvas-cont')[0].classList.add('epic');
		}
	}
	...
	function exportChest(mode) {
	var i, k, obj, res;
	res = '/setblock ~0 ~1 ~0 minecraft:chest 0 replace {Items:['
	if (mode == 0) {
			if (localStorage.getObject('bn').length < 1 || localStorage.getObject('bn') == null) {
				alert('Nothing to export.');
				return;
			}
			obj = localStorage.getObject('bn');
			for(var i=0;i<obj.length;i++) {
				res += '{id:minecraft:banner,Count:1,Slot:'+i+',tag:{BlockEntityTag:'+JSON.stringify(obj[i])+'}},';		
			}
			res += ']}';
		}else{
			obj = [];
			for(i=0;i<_savedTemp.length;i++){
				obj.push({Base:_savedTemp[i][0][0],Patterns:[]})
				for(k=1;k<_savedTemp[i].length;k++) {
					obj[i].Patterns.push({Color:_savedTemp[i][k][0],Pattern:_savedTemp[i][k][1]});
				}
			}
			for(var i=0;i<obj.length;i++) {
				res += '{id:minecraft:banner,Count:1,Slot:'+i+',tag:{BlockEntityTag:'+JSON.stringify(obj[i])+'}},';		
		...
		res = res.replace(/"/g,'');
		prompt('Your chest code is below. Copy this to a command block and activate it!',res);
	}
...
function compact(event) {
	if (event.target.classList.contains('on')) {
		...
		if(supports_html5_storage()) localStorage.compact = 'off';
	}else{
		event.target.parentNode.classList.add('compact');
		event.target.classList.add('on');
		if(supports_html5_storage()) localStorage.compact = 'on';
	}
}
...
function epic(event) {
	if (event.target.classList.contains('on')) {
		document.getElementsByClassName('canvas-cont')[0].classList.remove('epic');
		event.target.classList.remove('on');
		if(supports_html5_storage()) localStorage.epic = 'off';
	}else{
		document.getElementsByClassName('canvas-cont')[0].classList.add('epic');
		event.target.classList.add('on');
		if(supports_html5_storage()) localStorage.epic = 'on';
	}
}
